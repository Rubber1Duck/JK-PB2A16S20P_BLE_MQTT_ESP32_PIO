...
void handleBmsPage(WebServer &server)
{
    ...
    server.sendContent("<script>"
        ...
        "function buildCells(d){var g=document.getElementById('cg'),h='',a=parseFloat(d.cells.vol_ave);for(var i=0;i<32;i++){var b=(d.cells.sta>>i)&1,v=parseFloat(d.cells.vol[i]);if(!b)continue;var c=b?cc(v,a):'off';if((i+1)==d.cells.max_cell)c+=' mx';if((i+1)==d.cells.min_cell)c+=' mn';h+='<div class=\"cell '+c+'\"><div class=\"cn\">Z'+(i+1)+'</div><div class=\"cv\">'+d.cells.vol[i]+'</div></div>';}g.innerHTML=h;}"
